"""
Creates a plot in the sub-folder 'balancePlot' that summarizes the achievements of the program and gives an estimate for
...
def plot(balance, deposits=0, broker=''):
    """
    The function that generates the plot and writes it to the disk.
    :param broker: Used to allow for multiple brokers
    ...
    try:
        os.mkdir(broker + 'balancePlot')
    except FileExistsError:
        pass

    path = pathlib.Path('.').joinpath(broker + 'balancePlot')
    ...
    if len(balance_list) > 3:
        ...
        for index, point in enumerate(balance_list):
            ...
            temp = __get_earnings_cumulated(daily_earnings_list, point[0])
            daily_earnings_cumulated += [(point[0], temp)]
            tax_list += [(point[0], __get_tax(temp))]
            deposits_cumulated += [(point[0], __cumulated_deposits(deposits_list, point[0]))]
...
def __cumulated_deposits(deposits_list, today):
    """
...
def __get_earnings_cumulated(earnings, today):
    """
...
def __get_tax(earnings_cumulated):
    """
    Calculates the tax.
    """
    # todo: if the earnings from stocks are the only income for the current year, then taxes apply only after
    #  9744€ in earnings
